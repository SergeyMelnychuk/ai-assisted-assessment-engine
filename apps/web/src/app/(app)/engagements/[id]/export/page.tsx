...
import { ExportList } from "@/components/assessment/export-list";
...
export default async function ExportPage({
      ...
      <div className="space-y-4">
        ...
        <ul className="space-y-2">
          {resolved.engagement.assessments.map((a) => (
            <li key={a.id}>
              <Link
                href={`/engagements/${engagementId}/export?assessmentId=${a.id}`}
  ...
  return (
    <div className="space-y-6">
      ...
      <ExportList assessmentId={resolved.match.id} />
    </div>
  );
}
